...
for i = 1:numOfNtr
    ...
    filter = (zadoffChuTrainingSequence')/(zadoffChuTrainingSequence*zadoffChuTrainingSequence');
    for j = 0:40 
        Ex = 10^(j/10);
        sumOfBER = 0;
        for k = 1:numOfIteration
            ...
            yTraining = y(1:Ntr);
            hHat = channelEstimationInFreqFlatWithLS(filter, yTraining); % zadoffChuTrainingSequence is not multiplied with sqrt(Ex)
